function [ shortDist, mShortDist, mShortPath ] = CalcShortestPathDyn( mDistMtx, numRows, numCols )
% ----------------------------------------------------------------------------------------------- %
...
for jj = 2:numCols
    for ii = 2:numRows
        ...
        % vBaseDirections = [(ii - 1), jj; (ii - 1), (jj - 1); ii, (jj - 1)];
        ...
        % mShortPath(ii, jj) = ((vBaseDirections(minPathIdx, 2) - 1) * numRows) + vBaseDirections(minPathIdx, 1);
        
        mShortDist(ii, jj) = currLocDist + minPathVal;
    end
end
