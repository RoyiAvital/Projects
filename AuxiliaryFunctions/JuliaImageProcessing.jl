...
function _Conv2DSame!( mO :: Matrix{T}, mI :: Matrix{T}, mK :: Matrix{T} ) where {T <: AbstractFloat}
    ...
    for jj ∈ 1:numColsI
        for ii in 1:numRowsI
            sumVal = zero(T);
            for nn ∈ 1:numColsK, mm ∈ 1:numRowsK
                inBnd = (jj < numColsI + nn) && (jj > nn) && (ii < numRowsI + mm) && (ii > mm);
                @inbounds valI = inBnd ? mI[ii - mm + numRowsK, jj - nn + numColsK] : zero(T);
                @inbounds sumVal += mK[mm, nn] * valI; #mI[ii - mm + numRowsK, jj - nn + numColsK];
            end
            mO[ii, jj] = sumVal;
        end
    end
